...
if(!process.argv.includes('--icons')){
 for(const f of ['manifest.webmanifest','icon-192.png','icon-512.png'])writeFileSync(root+'docs/'+f,readFileSync(root+'public/'+f));
 ...
 const files=walk(root+'docs').filter(f=>f!=='sw.js');const version=createHash('sha256').update(files.map(f=>readFileSync(root+'docs/'+f)).join('')).digest('hex').slice(0,12);
 writeFileSync(root+'docs/sw.js',`const CACHE='night-shift-${version}';const FILES=${JSON.stringify(files.map(f=>'/'+f))};self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES)));});self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('night-shift-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));});self.addEventListener('fetch',e=>{if(e.request.method!=='GET'||new URL(e.request.url).origin!==self.location.origin)return;e.respondWith(caches.match(e.request).then(hit=>hit||fetch(e.request).catch(()=>e.request.mode==='navigate'?caches.match('/index.html'):Response.error())));});`);
